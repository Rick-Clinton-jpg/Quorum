...
- **Embedding backend defaults to offline hashing, not the real
  sentence-transformer — a deliberate default, not just a fallback.**
  The build spec calls for `sentence-transformers` / `all-MiniLM-L6-v2`.
  `intent_layer/extractor.py` used to try the real model first on every
  cold start; measured live (this repo's dev sandbox and the deployed
  Cloud Run service both pay the same cost) that cost ~6.6s per cold
  worker, and ~6s of that was `import sentence_transformers` itself (it
  pulls in torch) — paid in full even though the network call that
  follows was guaranteed to fail under the real egress policy (`403
  Forbidden` on `huggingface.co:443`, confirmed as a deliberate policy
  block, not a timeout or outage, via the proxy's own status endpoint).
  So `_get_model()` now goes straight to the offline scikit-learn
  `HashingVectorizer` embedding (word 1-2-grams) by default — 0.07s
  instead of 6.6s on the same measurement, no import, no network attempt
  — and only tries the real model if `QUORUM_INTENT_EMBEDDING_BACKEND`
  is explicitly set to `sentence-transformers`, e.g. in an environment
  whose egress policy actually permits `huggingface.co`. The eval below
  is unaffected either way, since the fallback is deterministic and this
  was already what every prior eval run actually used. The hashing
  backend is lexical, not semantic, which is *why* the graph-linking
  ...
  alone. `extractor.embedding_backend()` reports which backend actually
  ran, so this is easy to re-check from any environment.
